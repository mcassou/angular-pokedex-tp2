...
export class PokemonService {
  ...
  */
  ...
  /*
  * On récupère la liste de pokemons via la requete get sur l'API en envoyant l'offset, la limite et la valeur de la recherche reçus
  * Si la requête tombe en erreur on appelle la fonction handleError
  */
  getPokemonsBySearch(offset: number, limit: number,search: string): Observable<PagedData<Pokemon>> {
    return this.http.get<PagedData<Pokemon>>(this.pokemonsUrl + "?offset=" + offset + "&limit=" + limit + "&search=" + search).pipe(
      catchError(error => this.handleError<any>('getPokemonsBySearch', error))
    );
  }
